...
describe("Streamfund", function () {
  ...
  describe("Live ads with ETH", function () {
    this.beforeEach(async function () {
      const { accounts, deployedERC20, deployedPriceFeed, streamfund, owner } =
        await this.loadFixture(deployStreamfundFixture);
    ...
    it("Should failed to live ads because streamer is not registered", async function () {
      await expect(
        this.streamfund.connect(this.accounts[1]).liveAdsWithETH(this.accounts[5].address, "Thanks", {
          value: parseEther("1"),
        }),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });

    it("Should failed to live ads because message is too long", async function () {
      await expect(
        this.streamfund.connect(this.accounts[1]).liveAdsWithETH(this.accounts[0].address, "a".repeat(200), {
          value: parseEther("1"),
        }),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });

    it("Should failed to live ads because streamer have not set live ads price", async function () {
      await expect(
        this.streamfund.connect(this.accounts[1]).liveAdsWithETH(this.accounts[0].address, "Thanks", {
          value: parseEther("1"),
        }),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });

    it("Should failed to live ads because insufficient value", async function () {
      await this.streamfund.connect(this.accounts[0]).updateLiveAdsPrice(100000);
      await expect(
        this.streamfund.connect(this.accounts[1]).liveAdsWithETH(this.accounts[0].address, "Thanks", {
    ...
    it("Should live ads perfectly", async function () {
      await this.streamfund.connect(this.accounts[0]).updateLiveAdsPrice(100);
      const streamerPreBalance = await ethers.provider.getBalance(this.accounts[0]);
      ...
      ).to.be.emit(this.streamfund, "LiveAdsReceived");
  ...
  describe("Live ads with Token", function () {
    this.beforeEach(async function () {
      const { accounts, deployedERC20, deployedPriceFeed, streamfund, owner } =
        await this.loadFixture(deployStreamfundFixture);
    ...
    it("Should failed to live ads because streamer is not registered", async function () {
      ...
      await expect(
        this.streamfund
          .connect(this.accounts[1])
          .liveAdsWithToken(this.accounts[5].address, tokenAddr, BigInt(1 * 10 ** 18), "Thanks"),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });

    it("Should failed to live ads because token is not allowed", async function () {
      const tokenAddr = await this.deployedERC20[1].getAddress();
      await expect(
        this.streamfund
          .connect(this.accounts[1])
          .liveAdsWithToken(this.accounts[0].address, tokenAddr, BigInt(1 * 10 ** 18), "Thanks"),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });
    ...
    it("Should failed to live ads because message is too long", async function () {
      ...
      await expect(
        this.streamfund
          .connect(this.accounts[1])
          .liveAdsWithToken(this.accounts[0].address, tokenAddr, BigInt(1 * 10 ** 18), "a".repeat(200)),
      ).to.be.revertedWithCustomError(this.streamfund, "StreamfundValidationError");
    });
    ...
    it("Should failed to live ads because insufficient token to given", async function () {
      const tokenAddr = await this.deployedERC20[0].getAddress();
      await this.streamfund.connect(this.accounts[0]).updateLiveAdsPrice(100000);
      await expect(
        this.streamfund
    ...
    it("Should failed to live ads because token allowance is not enough", async function () {
      const tokenAddr = await this.deployedERC20[0].getAddress();
      await this.streamfund.connect(this.accounts[0]).updateLiveAdsPrice(100);
      await expect(
        this.streamfund
    ...
    it("Should live ads perfectly", async function () {
      const tokenAddr = await this.deployedERC20[0].getAddress();
      await this.streamfund.connect(this.accounts[0]).updateLiveAdsPrice(100);
      const streamerPreBalance = await this.deployedERC20[0].balanceOf(this.accounts[0]);
      ...
      const streamerPostBalance = await this.deployedERC20[0].balanceOf(this.accounts[0]);
      expect(streamerPostBalance).to.be.equal(streamerPreBalance + BigInt(1 * 10 ** 18));
    });
  });
});
